...
class FastSpeech2(nn.Module):
    def __init__(self, dictionary, out_dims=None):
        ...
        self.encoder = FS_ENCODERS['fft'](hparams, self.encoder_embed_tokens, self.dictionary)
        # self.decoder = FS_DECODERS[hparams['decoder_type']](hparams)
        self.decoder = FS_DECODERS['fft'](hparams)
    ...
    def forward(self, txt_tokens, mel2ph=None, spk_embed=None,
        ...
        ret = {}
        encoder_out = self.encoder(txt_tokens, **kwargs)  # [B, T_txt, H]
        src_nonpadding = (txt_tokens > 0).float()[:, :, None] # [B, T_txt, 1]
        ...
        mel2ph_ = mel2ph[..., None].repeat([1, 1, encoder_out.shape[-1]]) # [B, T_mel, H]
        decoder_inp_origin = decoder_inp = torch.gather(decoder_inp, 1, mel2ph_)  # [B, T_mel, H]
    ...
    def add_dur(self, dur_input, mel2ph, txt_tokens, ret):
        ...
        dur_input = dur_input.detach() + hparams['predictor_grad'] * (dur_input - dur_input.detach()) # 直通估计
        if mel2ph is None:
    ...
    def add_pitch(self, decoder_inp, f0, uv, mel2ph, ret, encoder_out=None):
        # 需要给函数输入ground trouth F0，因为模型有好几个部分都有各自的损失函数，
        # 需要分别训练，我们希望每个部分的输入都是理想的，这样才能避免误差传播，把每个部分训练好
        if hparams['pitch_type'] == 'ph':
            # detach的作用是避免这个predictor的损失影响到主模型，而是只引导这个predictor的参数更新
            pitch_pred_inp = encoder_out.detach() + hparams['predictor_grad'] * (encoder_out - encoder_out.detach()) # 直通估计
            pitch_padding = encoder_out.sum().abs() == 0
            ret['pitch_pred'] = pitch_pred = self.pitch_predictor(pitch_pred_inp) # normed f0
            if f0 is None:
                f0 = pitch_pred[:, :, 0]
    ...
    def run_decoder(self, decoder_inp, tgt_nonpadding, ret, infer, **kwargs):
        x = decoder_inp  # [B, T, H]
        x = self.decoder(x) # [B, T, H]
        x = self.mel_out(x) # [B, T, M]
