...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, mask, cond, clip_denoised: bool):
        noise_pred = self.denoise_fn(x, t, cond, mask)
    ...
    # 后向去噪
    @torch.no_grad()
    def p_sample(self, x, t, mask, cond, clip_denoised=True, repeat_noise=False):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, mask=mask, cond=cond, clip_denoised=clip_denoised)
    ...
    def p_losses(self, x_start, t, cond, noise=None, nonpadding=None):
        ...
        x_recon = self.denoise_fn(x_noisy, t, cond, nonpadding)
    ...
    def forward(self, txt_tokens, mel2ph=None, spk_embed=None,
        ...
        if not infer:
            ...
            x = ref_mels
            x = self.norm_spec(x)
            mask = (mel2ph != 0).unsqueeze(1).float()
            mask = mask.to(device)
            x = x.transpose(1, 2)[:, None, :, :]  # [B, 1, M, T]
            # 主要函数
            ret['diff_loss'] = self.p_losses(x_start=x, t=t, cond=cond, nonpadding=mask)
            ...
            for i in tqdm(reversed(range(0, t)), desc='sample time step', total=t):
                x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long), mask, cond)
            x = x[:, 0].transpose(1, 2)
...
class OfflineGaussianDiffusion(GaussianDiffusion):
    def forward(self, txt_tokens, mel2ph=None, spk_embed=None,
        ...
        if not infer:
            ...
            if hparams.get('gaussian_start') is not None and hparams['gaussian_start']:
                print('===> gaussion start.')
                shape = (cond.shape[0], 1, self.mel_bins, cond.shape[2]) # 拿到目标噪声的形状 [B, 1, M, T_mel]
                x = torch.randn(shape, device=device)
            for i in tqdm(reversed(range(0, t)), desc='sample time step', total=t):
