...
def process_utterance(wav_path,
    ...
    if isinstance(wav_path, str):
        if trim_long_sil:
            ...
            wav, _ = librosa.core.load(wav_path, sr=sample_rate) # 加载音频
    else:
        wav = wav_path
    ...
    if loud_norm:
        ...
        wav = pyln.normalize.loudness(wav, loudness, -22.0) # 归一化响度
        if np.abs(wav).max() > 1:
    ...
    spc = np.abs(x_stft)  # 计算复数模长

    # get mel basis
    fmin = 0 if fmin == -1 else fmin
    fmax = sample_rate / 2 if fmax == -1 else fmax # 香农采样定理
    # ( num_mels, 1 + fft_size / 2 )
    mel_basis = librosa.filters.mel(sr=sample_rate, n_fft=fft_size, n_mels=num_mels, fmin=fmin, fmax=fmax)
    mel = mel_basis @ spc # 梅尔滤波器组乘以线性谱图得到梅尔谱图

    if vocoder == 'pwg':
        mel = np.log10(np.maximum(eps, mel))  # (n_mel_bins, T)，转换成分贝(近似)
    else:
...
def get_mel2ph(tg_fn, ph, mel, hparams):
    ...
    tg_idx = 0
    ph_idx = 0
    tg_align = [x for x in tg['tiers'][-1]['items']] # 第二个tier记录音素时长信息
    tg_align_ = []
    for x in tg_align:
        x['xmin'] = float(x['xmin'])
        x['xmax'] = float(x['xmax'])
        if x['text'] in ['sil', 'sp', '', 'SIL', 'PUNC']:# sil代表silence
            x['text'] = ''
            if len(tg_align_) > 0 and tg_align_[-1]['text'] == '':
                tg_align_[-1]['xmax'] = x['xmax'] # 合并静默区
                continue
        tg_align_.append(x)
    tg_align = tg_align_
    ...
    while tg_idx < len(tg_align) or ph_idx < len(ph_list): # 二者的长度不一样
        ...
        if x['text'] != '' and is_sil_phoneme(ph): # 音素序列碰到分隔符则跳过
            ph_idx += 1
        else:
            ...
            if ph_idx > 0 and split[ph_idx - 1] == -1 and is_sil_phoneme(ph_list[ph_idx - 1]):
                split[ph_idx - 1] = split[ph_idx] # 令分隔符时长为零
            ph_idx += 1
            tg_idx += 1
