...
class FastSpeech2Task(TtsTask):
    ...
    def l2_loss(self, decoder_output, target):
        # decoder_output : B x T x n_mel
        # target : B x T x n_mel
        l2_loss = F.mse_loss(decoder_output, target, reduction='none')
        weights = self.weights_nonzero_speech(target)
    ...
    def add_dur_loss(self, dur_pred, mel2ph, txt_tokens, losses=None):
        ...
        dur_gt = mel2ph_to_dur(mel2ph, T).float() * nonpadding # 返回每个音素的帧数
    ...
    def after_infer(self, predictions):
        ...
        for num_predictions, prediction in enumerate(t):
            ...
            f0_gt = prediction.get("f0") - 5.
            f0_pred = prediction.get("f0_pred")
            midi_gt = prediction.get("midi_gt")
            ...
            if midi_gt is not None:
                if len(midi_gt) > len(mel_pred_mask):
                    midi_gt = midi_gt[:len(mel_pred_mask)]
                midi_gt = midi_gt[mel_pred_mask]
            ...
            if not hparams['profile_infer']:
                ...
                if mel_gt is not None and hparams['save_gt']:
                    ...
                    if hparams['save_f0']:
                        ...
                        plt.plot(f0_gt, label=r'$f0_G$')
                        if midi_gt is not None: 
                            plt.plot(midi_gt, label=r'$Midi_G$')
                        if hparams.get('pe_enable') is not None and hparams['pe_enable']:
