...
class BaseTrainer:
    ...
    # MODEL TRAINING
    # -----------------------------
    def fit(self, model): # model指的是task
        if self.use_ddp:
    ...
    def restore_training_state(self, checkpoint):
        ...
        if not hparams['reset_training_step']:
        ...
        if not hparams['reset_optimizer']:
        ...
        if not hparams['reset_lr_scheduler']:
    ...
    def train(self):
        model = self.get_model() # 获得task类模型
        # run all epochs
        ...
        for split_idx, split_batch in enumerate(splits):
            ...
            for opt_idx, optimizer in enumerate(self.optimizers):
                ...
                def optimizer_closure():
                    ...
                    closure_loss = closure_loss / self.accumulate_grad_batches # 在累计数上平均损失
    ...
    def training_forward(self, batch, batch_idx, opt_idx, hiddens):
        ...
        # distributed forward
        if self.use_ddp or self.use_dp:
            output = self.model(*args) ## 此时model.module才是真正的模型
        # single GPU forward
        elif self.single_gpu:
            gpu_id = 0
            if isinstance(self.data_parallel_device_ids, list):
